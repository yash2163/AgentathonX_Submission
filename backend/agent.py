...
class InfraAgent:
    ...
    def load_full_state(self):
        ...
        base_dir = self.get_graph_dir()
        print(f"DEBUG: Loading state from {base_dir}")
        try:
            # Load Implementation (Source of Truth for execution)
            impl_path = os.path.join(base_dir, "implementation_graph.json")
            print(f"DEBUG: Checking for implementation graph at {impl_path}")
            if os.path.exists(impl_path):
                print(f"DEBUG: Loading implementation graph...")
                with open(impl_path, "r") as f:
                    data = json.load(f)
                    print(f"DEBUG: Loaded {len(data.get('resources', []))} resources from file")
                    self.implementation_graph = GraphState(**data)
                    print(f"DEBUG: Calling load_nx_graph with {len(self.implementation_graph.resources)} resources")
                    self.load_nx_graph(self.implementation_graph)
                    print(f"DEBUG: NetworkX graph now has {self.graph.number_of_nodes()} nodes")
            else:
                print(f"DEBUG: No implementation graph found")
    ...
    def apply_policies_gen(self, intent_graph: GraphState) -> Generator[Any, None, None]:
        ...
        while cycle < max_cycles:
            ...
            if not new_data:
                yield ("log", "❌ Policy Engine failed after max retries.")
                return
    ...
    def expand_architecture_gen(self, reasoned_graph: GraphState, execution_mode: str) -> Generator[Any, None, None]:
        ...
        while api_attempts < max_api_attempts:
            try:
                ...
                if "503" in error_str or "429" in error_str:
                     api_attempts += 1
                     msg = f"⚠️ API Busy (Expand Attempt {api_attempts}/{max_api_attempts}). Retrying in 5s..."
                     print(f"DEBUG: {msg} Error: {error_str}")
                     yield ("log", msg)
                     time.sleep(5)
                else:
                    raise e
        ...
        if not data:
             yield ("log", "❌ Expansion failed after max retries.")
             return
        ...
        try:
            ...
            while api_attempts < max_api_attempts:
                try:
                    ...
                    if "503" in error_str or "429" in error_str:
                        api_attempts += 1
                        msg = f"⚠️ API Busy (Attempt {api_attempts}/{max_api_attempts}). Retrying Phase 1..."
                        print(f"DEBUG: {msg} Error: {error_str}")
                        yield send("log", msg)
                        time.sleep(5)
    ...
    def simulate_blast_radius(self, target_node_id: str) -> List[str]:
        ...
        Returns a list of affected node IDs (resources that depend on this node).
        
        NOTE: Graph edges represent dependencies (child -> parent), so we use
        ancestors() to find what depends on the target node.
        """
        if not self.graph.has_node(target_node_id):
            print(f"DEBUG: Node {target_node_id} not found in graph")
            return []
        
        try:
            # Graph Analysis: Comprehensive blast radius includes:
            # - Ancestors: nodes that depend on this (would break if this fails)
            # - Descendants: nodes this depends on (context of what this impacts)
            ancestors = set(nx.ancestors(self.graph, target_node_id))
            descendants = set(nx.descendants(self.graph, target_node_id))
            graph_affected = list(ancestors | descendants)
            
            print(f"DEBUG: Blast radius for {target_node_id}: {len(ancestors)} ancestors + {len(descendants)} descendants = {len(graph_affected)} total")
            
            # Optional: Also ask AI for semantic understanding
            try:
                current_state = self.export_state().model_dump_json()
                ...
                data = json.loads(response.text)
                ai_affected = data.get("affected_node_ids", [])
                
                print(f"DEBUG: AI found {len(ai_affected)} affected nodes")
                
                # Return union of both analyses
                return list(set(graph_affected + ai_affected))
                
            except Exception as ai_error:
                print(f"DEBUG: AI analysis failed ({ai_error}), using graph analysis only")
                # Fallback to graph analysis only
                return graph_affected

        except Exception as e:
            print(f"ERROR: Blast radius calculation failed: {e}")
            return []
